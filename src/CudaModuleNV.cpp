...
GPU_API VkResult GPU_CALL gpuCudaModuleNVCreate(
    ...
    *module = nullptr;
    ...
    const VkResult result = vkCreateCudaModuleNV(device->handle, createInfo, allocator, &handle);
    ...
    if (wrapper == nullptr) {
        vkDestroyCudaModuleNV(device->handle, handle, allocator);
...
GPU_API void GPU_CALL gpuCudaModuleNVDestroy(GPUCudaModuleNV module) {
    ...
    vkDestroyCudaModuleNV(module->device->handle, module->handle, module->allocator);
